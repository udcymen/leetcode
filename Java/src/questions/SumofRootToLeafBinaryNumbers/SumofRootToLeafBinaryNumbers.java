...
public class SumofRootToLeafBinaryNumbers {
     ...
     * if (root.left == null && root.right == null){ results.add((path)); } else{ if
     * (root.left != null){ dfs(root.left, path, results); } if (root.right !=
     * null){ dfs(root.right, path, results); } } }
     * 
     * public static int sumRootToLeaf(TreeNode root) { int result = 0;
     * 
     * if (root == null){ return result; }
     * 
     * List<String> results = new ArrayList<String>(); dfs(root, "", results);
     * 
     * for(String s : results){ result += Integer.parseInt(s, 2); }
     * 
     * return result; }
     */
